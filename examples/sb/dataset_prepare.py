...
def prepare_data(
    ...
    def get_mapped_label(e):
        return label_map.get(e, label_map.get(e.lower(), label_map.get(e.capitalize(), e)))

    new_train_data = {}
    for key, values in train_data.items():
        emo = values['emo']
        values['emo'] = get_mapped_label(emo)
...
def split_sets(train_data, split_ratio):
    ...
    ratio = split_ratio[0] / 100.0 if split_ratio[0] > 1.0 else split_ratio[0]
    num_train_nodev_samples = int(num_train_data * ratio)
    ...
    split_train_data = {key: train_data[key] for key in train_nodev_keys}
    valid_data = {key: train_data[key] for key in valid_keys}
    return split_train_data, valid_data
