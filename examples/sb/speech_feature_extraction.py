...
def load_audio_file(wav_path, start_time=None, end_time=None):
    try:
        ...
        if start_time is not None and end_time is not None:
            ...
            audio, sr = sf.read(wav_path, start=start_frame, stop=stop_frame, dtype="float32")
        else:
...
def extract_whisper_feature(wav_path, channel, model, output_norm, all_layers,device, start_time = None, end_time = None):
    ...
    wav, sr = load_audio_file(wav_path, start_time=start_time, end_time=end_time)
    channel = channel -1
    wav = wav[channel, :]
    if sr != SAMPLING_RATE:
...
def get_source(model_name):
    if model_name.startswith('wavlm'):
        ...
        source = 'facebook/hubert-base-ls960' if FairseqHubert is None else 'https://dl.fbaipublicfiles.com/hubert/hubert_base_ls960.pt'
...
if __name__ == '__main__':
    ...
    if 'hubert' in args.model_name and FairseqHubert is not None and source.endswith('.pt'):
